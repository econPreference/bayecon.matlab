             ...
             const emxArray_real_T *B, emxArray_real_T *Y)
{
  ...
  emxInit_real_T(sp, &b_A, 2, &cf_emlrtRTEI);
  st.site = &lf_emlrtRSI;
  i = b_A->size[0] * b_A->size[1];
  b_A->size[0] = A->size[0];
  b_A->size[1] = A->size[1];
  emxEnsureCapacity_real_T(&st, b_A, i, &te_emlrtRTEI);
  b_A_data = b_A->data;
  na = A->size[0] * A->size[1];
  for (i = 0; i < na; i++) {
    b_A_data[i] = A_data[i];
  }
  emxInit_int32_T(&st, &jpvt, 2, &cf_emlrtRTEI);
  minmn = b_A->size[0];
  b_na = b_A->size[1];
  i = jpvt->size[0] * jpvt->size[1];
  jpvt->size[0] = 1;
  jpvt->size[1] = b_A->size[1];
  emxEnsureCapacity_int32_T(&st, jpvt, i, &ue_emlrtRTEI);
  jpvt_data = jpvt->data;
  na = b_A->size[1];
  for (i = 0; i < na; i++) {
    jpvt_data[i] = 0;
  }
  emxInit_real_T(&st, &tau, 1, &cf_emlrtRTEI);
  ...
  i = tau->size[0];
  tau->size[0] = minmana;
  emxEnsureCapacity_real_T(&b_st, tau, i, &ve_emlrtRTEI);
  tau_data = tau->data;
  if ((b_A->size[0] == 0) || (b_A->size[1] == 0)) {
    i = tau->size[0];
    tau->size[0] = minmana;
    emxEnsureCapacity_real_T(&b_st, tau, i, &xe_emlrtRTEI);
    tau_data = tau->data;
    for (i = 0; i < minmana; i++) {
      tau_data[i] = 0.0;
    }
    ...
    emxInit_ptrdiff_t(&b_st, &jpvt_t, &we_emlrtRTEI);
    i = jpvt_t->size[0];
    jpvt_t->size[0] = b_A->size[1];
    emxEnsureCapacity_ptrdiff_t(&b_st, jpvt_t, i, &we_emlrtRTEI);
  ...
  if (ma < minmn) {
    ...
    c_emlrt_marshallIn(&c_st, b_sprintf(&c_st, y, b_y, &t_emlrtMCI),
                       "<output of sprintf>", str);
    b_st.site = &yf_emlrtRSI;
    c_warning(&b_st, ma, str);
  }
  emxInit_real_T(&st, &b_B, 1, &ye_emlrtRTEI);
  st.site = &nf_emlrtRSI;
  i = b_B->size[0];
  b_B->size[0] = B->size[0];
  emxEnsureCapacity_real_T(&st, b_B, i, &ye_emlrtRTEI);
  b_B_data = b_B->data;
  na = B->size[0];
  for (i = 0; i < na; i++) {
    b_B_data[i] = B_data[i];
  ...
  i = Y->size[0];
  Y->size[0] = b_A->size[1];
  emxEnsureCapacity_real_T(&st, Y, i, &af_emlrtRTEI);
  Y_data = Y->data;
  na = b_A->size[1];
  for (i = 0; i < na; i++) {
    Y_data[i] = 0.0;
  ...
  if ((b_A->size[0] != 0) && (b_A->size[1] != 0) && (b_B->size[0] != 0)) {
    ...
    if (p) {
      ...
      i = b_B->size[0];
      b_B->size[0] = na;
      emxEnsureCapacity_real_T(&c_st, b_B, i, &bf_emlrtRTEI);
      b_B_data = b_B->data;
      for (i = 0; i < na; i++) {
        b_B_data[i] = rtNaN;
      }
