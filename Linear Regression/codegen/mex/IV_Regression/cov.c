...
void cov(const emlrtStack *sp, const emxArray_real_T *x, emxArray_real_T *xy)
{
  ...
  if (x->size[0] == 1) {
    emxInit_real_T(sp, &b_x, 1, &xg_emlrtRTEI);
    st.site = &hh_emlrtRSI;
    LDA = x->size[1];
    i = b_x->size[0];
    b_x->size[0] = x->size[1];
    emxEnsureCapacity_real_T(&st, b_x, i, &xg_emlrtRTEI);
    b_x_data = b_x->data;
    for (i = 0; i < LDA; i++) {
    ...
    xy->size[0] = 1;
    xy->size[1] = 1;
    emxEnsureCapacity_real_T(sp, xy, i, &ah_emlrtRTEI);
    xy_data = xy->data;
    xy_data[0] = c;
  } else {
    emxInit_real_T(sp, &b_x, 2, &wg_emlrtRTEI);
    st.site = &ih_emlrtRSI;
    i = b_x->size[0] * b_x->size[1];
    b_x->size[0] = x->size[0];
    b_x->size[1] = x->size[1];
    emxEnsureCapacity_real_T(&st, b_x, i, &wg_emlrtRTEI);
    b_x_data = b_x->data;
    ...
    xy->size[0] = x->size[1];
    xy->size[1] = x->size[1];
    emxEnsureCapacity_real_T(&st, xy, i, &yg_emlrtRTEI);
    xy_data = xy->data;
