            ...
            int32_T lda, emxArray_int32_T *ipiv)
{
  ...
  if ((A->size[0] == 0) || (A->size[1] == 0)) {
    ...
    emxInit_ptrdiff_t(&st, &ipiv_t, &kg_emlrtRTEI);
    emxInit_ptrdiff_t(&st, &r, &gg_emlrtRTEI);
    i = muIntScalarMin_sint32(m, n);
    b_st.site = &sd_emlrtRSI;
    repmat(&b_st, (ptrdiff_t)0.0, muIntScalarMax_sint32(i, 1), r);
    i = ipiv_t->size[0];
    ipiv_t->size[0] = r->size[0];
    emxEnsureCapacity_ptrdiff_t(&st, ipiv_t, i, &gg_emlrtRTEI);
    ...
    emxEnsureCapacity_int32_T(&st, ipiv, i, &ig_emlrtRTEI);
    ipiv_data = ipiv->data;
    ...
    if (info < 0) {
      if (info == -1010) {
        ...
        emlrtErrorWithMessageIdR2018a(
            &b_st, &q_emlrtRTEI, "Coder:toolbox:LAPACKCallErrorInfo",
