...
void inv(const emlrtStack *sp, const emxArray_real_T *x, emxArray_real_T *y)
{
  ...
  if ((x->size[0] == 0) || (x->size[1] == 0)) {
    ...
    y->size[0] = x->size[0];
    y->size[1] = x->size[1];
    emxEnsureCapacity_real_T(sp, y, i, &be_emlrtRTEI);
    y_data = y->data;
    ...
    y->size[0] = x->size[0];
    y->size[1] = x->size[1];
    emxEnsureCapacity_real_T(&st, y, i, &ce_emlrtRTEI);
    y_data = y->data;
    yk = x->size[0] * x->size[1];
    for (i = 0; i < yk; i++) {
      y_data[i] = 0.0;
    }
    emxInit_real_T(&st, &b_x, 2, &ge_emlrtRTEI);
    i = b_x->size[0] * b_x->size[1];
    b_x->size[0] = x->size[0];
    b_x->size[1] = x->size[1];
    emxEnsureCapacity_real_T(&st, b_x, i, &de_emlrtRTEI);
    b_x_data = b_x->data;
    yk = x->size[0] * x->size[1];
    for (i = 0; i < yk; i++) {
      b_x_data[i] = x_data[i];
    }
    emxInit_int32_T(&st, &p, 2, &fe_emlrtRTEI);
    emxInit_int32_T(&st, &ipiv, 2, &ge_emlrtRTEI);
    ...
    p->size[0] = 1;
    p->size[1] = x->size[0];
    emxEnsureCapacity_int32_T(&f_st, p, i, &ee_emlrtRTEI);
    p_data = p->data;
    p_data[0] = 1;
    yk = 1;
