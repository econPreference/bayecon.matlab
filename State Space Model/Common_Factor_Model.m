function Output = Common_Factor_Model(y,h0,H0,g0,G0,a0,d0,scale,n0,n1)
...
for iter = 1:n
    ...
    for i = 1:N
        if i == 1
            H(i,1) = Gen_coef1(y(:,i), beta, sig2(i,1), h0(i,1), H0(i,1));
        else
...
function coef = Gen_coef1(Y, X, sig2, coef0, B0)
% res: Test "res"th parameter is positive or not. (res is column vector)
% If res is zeros, then test will not be counducted.
...
coef = trandn(B1*A,cholmod(B1),0,exp(200));
