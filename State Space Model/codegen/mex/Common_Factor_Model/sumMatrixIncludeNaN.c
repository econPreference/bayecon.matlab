...
static real_T e_sumColumnB(const emlrtStack *sp, const emxArray_real_T *x,
...
real_T c_sumColumnB(const real_T x[4], int32_T col)
{
  int32_T i0;
  i0 = (col - 1) << 1;
  return x[i0] + x[i0 + 1];
...
real_T f_sumColumnB(const real_T x[2])
{
  return x[0] + x[1];
}

real_T g_sumColumnB(real_T x)
{
  return x;
}
                        ...
                        int32_T vlen)
{
  ...
  if (vlen < 4096) {
    ...
    if (nleft > 0) {
      st.site = &yb_emlrtRSI;
      y += e_sumColumnB(&st, x, nleft, inb + 1);
    }
  }
  return y;
}
