...
void eig(const emlrtStack *sp, const emxArray_real_T *A, emxArray_creal_T *V)
{
  ...
  i = V->size[0];
  V->size[0] = A->size[0];
  emxEnsureCapacity_creal_T(sp, V, i, &jf_emlrtRTEI);
  if ((A->size[0] != 0) && (A->size[1] != 0)) {
    st.site = &mf_emlrtRSI;
    if (anyNonFinite(&st, A)) {
      i = V->size[0];
      V->size[0] = A->size[0];
      emxEnsureCapacity_creal_T(sp, V, i, &kf_emlrtRTEI);
      V_data = V->data;
      j = A->size[0];
      for (i = 0; i < j; i++) {
        V_data[i].re = rtNaN;
      ...
      emxInit_real_T(sp, &scale, 1, &tf_emlrtRTEI);
      emxInit_real_T(sp, &T, 2, &wf_emlrtRTEI);
      if (p) {
        st.site = &nf_emlrtRSI;
        ...
        emxEnsureCapacity_real_T(&b_st, scale, i, &lf_emlrtRTEI);
        ...
        for (j = 0; j < n; j++) {
          scale_data[j] = wreal_data[j + T->size[0] * j];
        }
        i = V->size[0];
        V->size[0] = scale->size[0];
        emxEnsureCapacity_creal_T(sp, V, i, &of_emlrtRTEI);
        V_data = V->data;
        j = scale->size[0];
        for (i = 0; i < j; i++) {
        ...
        if (p) {
          ...
          i = V->size[0];
          V->size[0] = T->size[0];
          emxEnsureCapacity_creal_T(&b_st, V, i, &nf_emlrtRTEI);
          V_data = V->data;
          i = 1;
          do {
            exitg1 = 0;
            if (i <= n) {
              guard1 = false;
              if (i != n) {
                vleft = wreal_data[i + T->size[0] * (i - 1)];
                if (vleft != 0.0) {
          ...
          emxInit_real_T(sp, &b_A, 2, &mf_emlrtRTEI);
          st.site = &pf_emlrtRSI;
          ...
          b_A->size[0] = A->size[0];
          b_A->size[1] = A->size[1];
          emxEnsureCapacity_real_T(&c_st, b_A, i, &mf_emlrtRTEI);
          b_A_data = b_A->data;
          j = A->size[0] * A->size[1];
          for (i = 0; i < j; i++) {
            b_A_data[i] = A_data[i];
          }
          emxInit_real_T(&c_st, &wreal, 1, &uf_emlrtRTEI);
          emxInit_real_T(&c_st, &wimag, 1, &vf_emlrtRTEI);
          n = A->size[1];
          i = scale->size[0];
          scale->size[0] = A->size[1];
          emxEnsureCapacity_real_T(&c_st, scale, i, &pf_emlrtRTEI);
          scale_data = scale->data;
          i = V->size[0];
          V->size[0] = A->size[1];
          emxEnsureCapacity_creal_T(&c_st, V, i, &qf_emlrtRTEI);
          V_data = V->data;
          i = wreal->size[0];
          wreal->size[0] = A->size[1];
          emxEnsureCapacity_real_T(&c_st, wreal, i, &rf_emlrtRTEI);
          wreal_data = wreal->data;
          i = wimag->size[0];
          wimag->size[0] = A->size[1];
          emxEnsureCapacity_real_T(&c_st, wimag, i, &sf_emlrtRTEI);
          wimag_data = wimag->data;
          info_t = LAPACKE_dgeevx(
              102, 'B', 'N', 'N', 'N', (ptrdiff_t)A->size[1], &b_A_data[0],
