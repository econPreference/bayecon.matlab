...
void d_binary_expand_op(const emlrtStack *sp, emxArray_real_T *Ainv)
{
  ...
  emxInit_real_T(sp, &b_Ainv, 2, &be_emlrtRTEI);
  ...
  emxEnsureCapacity_real_T(sp, b_Ainv, i, &be_emlrtRTEI);
  ...
  emxEnsureCapacity_real_T(sp, Ainv, i, &ce_emlrtRTEI);
  Ainv_data = Ainv->data;
           ...
           emxArray_real_T *Ainv)
{
  ...
  if (A->size[0] != A->size[1]) {
    ...
    emxInit_real_T(sp, &Hinv, 2, &fe_emlrtRTEI);
    st.site = &gd_emlrtRSI;
    ...
    /* output : Matrix T */
    n = Hinv->size[1];
    emxInit_real_T(&st, &a, 2, &de_emlrtRTEI);
    emxInit_real_T(&st, &b, 1, &ee_emlrtRTEI);
    if (Hinv->size[0] != n) {
      b_y = NULL;
      ...
      while ((!exitg1) && (k <= n - 1)) {
        ...
        if (d_sum == 0.0) {
          ...
          for (b_i = 0; b_i <= b_k - 2; b_i++) {
            ...
            loop_ub = i1 - i;
            a->size[1] = loop_ub;
            emxEnsureCapacity_real_T(&b_st, a, b_loop_ub, &de_emlrtRTEI);
            Ainv_data = a->data;
            ...
            i3 = b->size[0];
            b->size[0] = b_loop_ub;
            emxEnsureCapacity_real_T(&b_st, b, i3, &ee_emlrtRTEI);
            b_data = b->data;
    ...
    e_mtimes(&b_st, Hinv, Hinv, Ainv);
    Ainv_data = Ainv->data;
    if ((Ainv->size[0] != Ainv->size[1]) &&
        ((Ainv->size[0] != 1) && (Ainv->size[1] != 1))) {
      emlrtDimSizeImpxCheckR2021b(Ainv->size[0], Ainv->size[1], &x_emlrtECI,
                                  (emlrtCTX)sp);
    }
    if ((Ainv->size[0] != Ainv->size[1]) &&
        ((Ainv->size[1] != 1) && (Ainv->size[0] != 1))) {
      emlrtDimSizeImpxCheckR2021b(Ainv->size[1], Ainv->size[0], &w_emlrtECI,
                                  (emlrtCTX)sp);
    }
    if (Ainv->size[0] == Ainv->size[1]) {
      ...
      emxEnsureCapacity_real_T(sp, Hinv, i, &be_emlrtRTEI);
      Hinv_data = Hinv->data;
      loop_ub = Ainv->size[1];
      for (i = 0; i < loop_ub; i++) {
        b_loop_ub = Ainv->size[0];
