function Output = Auto_Unit(data,kk,vv,a0,d0,phi0,PHI0,n0,n1,mode,aa,bb,c0,C0)
...
box.aa = aa;
box.bb = bb;
Z_b = normC(@b_f,box,@b_g,box,@b_gs,box,30000);
...
for iter = 1:n
    ...
    if resid == 0
        clc
        disp('=====================================')
        disp('Under H1: this is an I(0) process.')
    ...
    [b,logprior_b] = Gen_beta(b,1,X_hat,Y_hat - X1_hat*c,X_p,Y_p - X1_p*c,kk,vv,inv_Sigma,sig2_inv,mode,aa,bb);
    bm(iter,:) = b';
    ...
    logprior = logprior_b - log(Z_b) + logprior_sig2 + logprior_phi + logprior_c;
...
function [beta1,logprior] = Gen_beta(beta0,theta0,X_hat,Y_hat,X_p,Y_p,k,v,inv_Sigma,sig2_inv,mode,aa,bb)

coder.extrinsic('makedist')
coder.extrinsic('truncate')

tau = tausetter(mode,theta0,v,k);
% b = (X_hat'*X_hat)\(X_hat'*Y_hat);
pd = makedist('Normal','mu',beta0,'sigma',0.1);
pdt = truncate(pd,aa,bb);

% weight with proposal
proposal = 0; %#ok<NASGU>
proposal = random(pdt);
loglik = loglik_kernel(proposal,X_p,Y_p,X_hat,Y_hat,sig2_inv,inv_Sigma);
logprior_proposal = logprior_kernel(proposal,theta0,k,v,tau);
proposal_numer = loglik + logprior_proposal;
proposal_deno = log(pdf(pdt,proposal));
weight_numer = proposal_numer - proposal_deno;

% weight with beta0
loglik = loglik_kernel(beta0,X_p,Y_p,X_hat,Y_hat,sig2_inv,inv_Sigma);
logprior_beta0 = logprior_kernel(beta0,theta0,k,v,tau);
beta0_numer = loglik + logprior_beta0;
beta0_deno = log(pdf(pdt,beta0));
weight_deno = beta0_numer - beta0_deno;

accept = 0; %#ok<NASGU>
accept = min(weight_numer - weight_deno,0);
if log(rand) < accept
    beta1 = proposal;
    logprior = logprior_proposal;
else
    beta1 = beta0;
    logprior = logprior_beta0;
end
...
function loglik = loglik_kernel(beta,X_p,Y_p,X_hat,Y_hat,sig2_inv,inv_Sigma)

loglik = -0.5*sig2_inv*(Y_p - X_p*beta)'*inv_Sigma*(Y_p - X_p*beta)...
    + (-0.5*sig2_inv*(Y_hat - X_hat*beta)'*(Y_hat - X_hat*beta));

end

function logprior = logprior_kernel(theta,theta0,k,v,tau)
...
function tau = tausetter(theta,theta0,v,k)

numer = (theta-theta0)^2;
deno = ((2*k)/(v+1))^(1/(2*k));
tau = numer/deno;
