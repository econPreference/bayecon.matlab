...
void cov(const emlrtStack *sp, const emxArray_real_T *x, emxArray_real_T *xy)
{
  ...
  if (x->size[0] == 1) {
    emxInit_real_T(sp, &b_x, 1, &lk_emlrtRTEI);
    st.site = &wk_emlrtRSI;
    LDA = x->size[1];
    i = b_x->size[0];
    b_x->size[0] = x->size[1];
    emxEnsureCapacity_real_T(&st, b_x, i, &lk_emlrtRTEI);
    b_x_data = b_x->data;
    for (i = 0; i < LDA; i++) {
    ...
    xy->size[0] = 1;
    xy->size[1] = 1;
    emxEnsureCapacity_real_T(sp, xy, i, &nk_emlrtRTEI);
    xy_data = xy->data;
    xy_data[0] = c;
  } else {
    emxInit_real_T(sp, &b_x, 2, &kk_emlrtRTEI);
    st.site = &xk_emlrtRSI;
    i = b_x->size[0] * b_x->size[1];
    b_x->size[0] = x->size[0];
    b_x->size[1] = x->size[1];
    emxEnsureCapacity_real_T(&st, b_x, i, &kk_emlrtRTEI);
    b_x_data = b_x->data;
    ...
    xy->size[0] = x->size[1];
    xy->size[1] = x->size[1];
    emxEnsureCapacity_real_T(&st, xy, i, &mk_emlrtRTEI);
    xy_data = xy->data;
