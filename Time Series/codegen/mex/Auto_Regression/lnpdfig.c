...
void lnpdfig(const emlrtStack *sp, const real_T sig2_data[], real_T alpha,
             real_T beta, real_T z_data[], int32_T *z_size)
{
  ...
  if (!muDoubleScalarIsNaN(alpha)) {
    if (alpha < 0.0) {
    ...
    } else if (alpha > 2.55E+305) {
      t = rtInf;
    } else if (alpha <= 2.2204460492503131E-16) {
      t = -muDoubleScalarLog(alpha);
    } else if (alpha <= 0.5) {
      t = 1.0;
      r = 0.0;
      for (i = 0; i < 8; i++) {
      ...
      t = -muDoubleScalarLog(alpha) +
          alpha * (alpha * (r / t) + -0.57721566490153287);
    } else if (alpha <= 0.6796875) {
      t = 1.0;
      r = 0.0;
      for (i = 0; i < 8; i++) {
        r = r * ((alpha - 0.5) - 0.5) + p2[i];
        t = t * ((alpha - 0.5) - 0.5) + q2[i];
      }
      t = -muDoubleScalarLog(alpha) +
          ((alpha - 0.5) - 0.5) *
              (((alpha - 0.5) - 0.5) * (r / t) + 0.42278433509846713);
    } else if ((alpha == muDoubleScalarFloor(alpha)) && (alpha <= 100.0)) {
      t = table100[(int32_T)alpha - 1];
    } else if (alpha <= 1.5) {
      t = 1.0;
      r = 0.0;
      for (i = 0; i < 8; i++) {
        r = r * ((alpha - 0.5) - 0.5) + p1[i];
        t = t * ((alpha - 0.5) - 0.5) + q1[i];
      }
      t = ((alpha - 0.5) - 0.5) *
          (((alpha - 0.5) - 0.5) * (r / t) + -0.57721566490153287);
    } else if (alpha <= 4.0) {
      t = 1.0;
      r = 0.0;
      for (i = 0; i < 8; i++) {
      ...
      t = (alpha - 2.0) * ((alpha - 2.0) * (r / t) + 0.42278433509846713);
    } else if (alpha <= 12.0) {
      t = -1.0;
      r = 0.0;
      for (i = 0; i < 8; i++) {
      ...
      t = (alpha - 4.0) * (r / t) + 1.791759469228055;
    } else {
      if (alpha <= 2.25E+76) {
        r = 0.0057083835261;
        t = 1.0 / (alpha * alpha);
      ...
      t = muDoubleScalarLog(alpha);
      t = ((r + 0.91893853320467278) - 0.5 * t) + alpha * (t - 1.0);
    }
  }
  st.site = &lk_emlrtRSI;
  z_data[0] = sig2_data[0];
  if (sig2_data[0] < 0.0) {
    emlrtErrorWithMessageIdR2018a(
        &st, &v_emlrtRTEI, "Coder:toolbox:ElFunDomainError",
        "Coder:toolbox:ElFunDomainError", 3, 4, 3, "log");
  ...
  z_data[0] = muDoubleScalarLog(z_data[0]);
  *z_size = 1;
  z_data[0] =
      ((alpha * muDoubleScalarLog(beta) - t) - (alpha + 1.0) * z_data[0]) -
      beta / sig2_data[0];
}
