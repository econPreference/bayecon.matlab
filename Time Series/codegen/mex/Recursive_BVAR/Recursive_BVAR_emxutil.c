 ...
 *
 * Recursive_BVAR_emxutil.c
 *
 * Code generation for function 'Recursive_BVAR_emxutil'
 *
 */
...
{
  emxFree_real_T(&pStruct->Bm);
  emxFree_real_T(&pStruct->consm);
  emxFree_real_T(&pStruct->Gammam);
  emxFree_real_T(&pStruct->sigmam);
  emxFree_real_T(&pStruct->ImpulseRespm);
}
...
{
  emxInit_real_T(sp, &pStruct->Bm, 2, srcLocation, doPush);
  emxInit_real_T(sp, &pStruct->consm, 2, srcLocation, doPush);
  emxInit_real_T(sp, &pStruct->Gammam, 2, srcLocation, doPush);
  emxInit_real_T(sp, &pStruct->sigmam, 2, srcLocation, doPush);
  emxInit_real_T2(sp, &pStruct->ImpulseRespm, 3, srcLocation, doPush);
}
...
/* End of code generation (Recursive_BVAR_emxutil.c) */
