             ...
             emxArray_real_T *r)
{
  ...
  emxEnsureCapacity_real_T(sp, r, i, &af_emlrtRTEI);
  if (!(varargin_2 >= 0.0)) {
    emlrtNonNegativeCheckR2012b(varargin_2, &pb_emlrtDCI, (emlrtCTX)sp);
  }
  i = r->size[0] * r->size[1];
...
void randn(const emlrtStack *sp, real_T varargin_1, emxArray_real_T *r)
{
  ...
  int32_T i;
  if (!(varargin_1 >= 0.0)) {
    emlrtNonNegativeCheckR2012b(varargin_1, &pb_emlrtDCI, (emlrtCTX)sp);
  }
  i = r->size[0];
  r->size[0] = (int32_T)varargin_1;
  emxEnsureCapacity_real_T(sp, r, i, &af_emlrtRTEI);
