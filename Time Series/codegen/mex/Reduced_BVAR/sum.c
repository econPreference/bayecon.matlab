...
void c_sum(const emlrtStack *sp, const emxArray_real_T *x, emxArray_real_T *y)
{
  ...
  if ((x->size[0] == 0) || (x->size[1] == 0)) {
    hi = y->size[0];
    y->size[0] = x->size[0];
    emxEnsureCapacity_real_T(&c_st, y, hi, &vf_emlrtRTEI);
    y_data = y->data;
    ...
    emxInit_real_T(&c_st, &bsum, 1, &yf_emlrtRTEI);
    d_st.site = &kf_emlrtRSI;
    vstride = x->size[0];
    bvstride = x->size[0] << 10;
    hi = y->size[0];
    y->size[0] = x->size[0];
    emxEnsureCapacity_real_T(&d_st, y, hi, &wf_emlrtRTEI);
    y_data = y->data;
    hi = bsum->size[0];
    bsum->size[0] = x->size[0];
    emxEnsureCapacity_real_T(&d_st, bsum, hi, &xf_emlrtRTEI);
    bsum_data = bsum->data;
    if (x->size[1] <= 1024) {
