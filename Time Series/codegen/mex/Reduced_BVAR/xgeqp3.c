            ...
            emxArray_int32_T *jpvt)
{
  ...
  emxEnsureCapacity_int32_T(sp, jpvt, i, &dh_emlrtRTEI);
  jpvt_data = jpvt->data;
  b_na = A->size[1];
  for (i = 0; i < b_na; i++) {
    jpvt_data[i] = 0;
  ...
  i = tau->size[0];
  tau->size[0] = minmana;
  emxEnsureCapacity_real_T(&st, tau, i, &eh_emlrtRTEI);
  tau_data = tau->data;
  emxInit_ptrdiff_t(&st, &jpvt_t, &fh_emlrtRTEI);
  if ((A->size[0] == 0) || (A->size[1] == 0) || (A->size[0] < 1) ||
      (A->size[1] < 1)) {
    i = tau->size[0];
    tau->size[0] = minmana;
    emxEnsureCapacity_real_T(&st, tau, i, &gh_emlrtRTEI);
    tau_data = tau->data;
    for (i = 0; i < minmana; i++) {
      tau_data[i] = 0.0;
    }
    ...
    emxEnsureCapacity_ptrdiff_t(&st, jpvt_t, i, &fh_emlrtRTEI);
    ...
    if (b_na != 0) {
      ...
      if (b_na != -4) {
        if (b_na == -1010) {
          emlrtErrorWithMessageIdR2018a(&b_st, &w_emlrtRTEI, "MATLAB:nomem",
                                        "MATLAB:nomem", 0);
        } else {
