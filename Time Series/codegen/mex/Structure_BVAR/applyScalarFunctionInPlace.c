...
#pragma omp parallel num_threads(emlrtAllocRegionTLSs(                         \
    ...
    omp_get_num_procs())) private(emlrtJBEnviron, e_st, t, d, r, xnum, i)      \
          ...
          if (t < 0.0) {
            ...
            if (d < 0.0) {
              emlrtErrorWithMessageIdR2018a(
                  &e_st, &ab_emlrtRTEI, "Coder:toolbox:ElFunDomainError",
                  "Coder:toolbox:ElFunDomainError", 3, 4, 3, "log");
            }
            t = -muDoubleScalarLog(d);
          } else {
            d = x_data[k];
            if (d <= 0.5) {
              r = 1.0;
              ...
              t = -muDoubleScalarLog(x_data[k]) +
                  d * (d * (xnum / r) + -0.57721566490153287);
            } else if (d <= 0.6796875) {
              r = 1.0;
              xnum = 0.0;
              for (i = 0; i < 8; i++) {
                xnum = xnum * ((t - 0.5) - 0.5) + p2[i];
                r = r * ((t - 0.5) - 0.5) + q2[i];
              }
              t = (x_data[k] - 0.5) - 0.5;
              t = -muDoubleScalarLog(x_data[k]) +
                  t * (t * (xnum / r) + 0.42278433509846713);
            } else if ((d == muDoubleScalarFloor(d)) && (d <= 100.0)) {
              t = table100[(int32_T)d - 1];
            } else if (d <= 1.5) {
              r = 1.0;
              xnum = 0.0;
              for (i = 0; i < 8; i++) {
                xnum = xnum * ((t - 0.5) - 0.5) + p1[i];
                r = r * ((t - 0.5) - 0.5) + q1[i];
              }
              t = (x_data[k] - 0.5) - 0.5;
              t *= t * (xnum / r) + -0.57721566490153287;
            } else if (d <= 4.0) {
              r = 1.0;
              xnum = 0.0;
              for (i = 0; i < 8; i++) {
                xnum = xnum * (t - 2.0) + p2[i];
                r = r * (t - 2.0) + q2[i];
              }
              t = (d - 2.0) * ((d - 2.0) * (xnum / r) + 0.42278433509846713);
            } else if (d <= 12.0) {
              r = -1.0;
              xnum = 0.0;
              for (i = 0; i < 8; i++) {
                xnum = xnum * (t - 4.0) + p4[i];
                r = r * (t - 4.0) + q4[i];
              }
              t = (d - 4.0) * (xnum / r) + 1.791759469228055;
            } else {
              if (d <= 2.25E+76) {
                r = 0.0057083835261;
                t = 1.0 / (d * d);
              ...
              t = muDoubleScalarLog(x_data[k]);
              t = ((r + 0.91893853320467278) - 0.5 * t) + d * (t - 1.0);
            }
          }
        }
        x_data[k] = t;
