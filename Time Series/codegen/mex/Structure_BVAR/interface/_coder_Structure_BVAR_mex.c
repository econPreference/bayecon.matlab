...
#include "Structure_BVAR_terminate.h"
                                ...
                                const mxArray *prhs[13])
{
  ...
  /* Call the function. */
  Structure_BVAR_api(prhs, &outputs);
                 ...
                 const mxArray *prhs[])
{
  ...
  mexAtExit(&Structure_BVAR_atexit);
  /* Initialize the memory manager. */
  omp_init_lock(&emlrtLockGlobal);
  omp_init_nest_lock(&Structure_BVAR_nestLockGlobal);
  /* Module initialization. */
  Structure_BVAR_initialize();
  st.tls = emlrtRootTLSGlobal;
  emlrtSetJmpBuf(&st, &emlrtJBEnviron);
  if (setjmp(emlrtJBEnviron) == 0) {
    /* Dispatch the entry-point. */
    Structure_BVAR_mexFunction(nlhs, plhs, nrhs, prhs);
    /* Module termination. */
    Structure_BVAR_terminate();
    omp_destroy_lock(&emlrtLockGlobal);
    omp_destroy_nest_lock(&Structure_BVAR_nestLockGlobal);
  } else {
    omp_destroy_lock(&emlrtLockGlobal);
    omp_destroy_nest_lock(&Structure_BVAR_nestLockGlobal);
...
/* End of code generation (_coder_Structure_BVAR_mex.c) */
