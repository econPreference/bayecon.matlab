                    ...
                    const emxArray_real_T *R, real_T v)
{
  ...
  emxInit_real_T(sp, &R_inv, 2, &xh_emlrtRTEI);
  ...
  emxInit_real_T(&st, &y, 2, &bi_emlrtRTEI);
  x_data = y->data;
  ...
  if (X->size[0] < 1) {
    ...
    y->size[0] = 1;
    y->size[1] = X->size[0];
    emxEnsureCapacity_real_T(&st, y, i, &uh_emlrtRTEI);
    x_data = y->data;
    bcoef = X->size[0] - 1;
    for (i = 0; i <= bcoef; i++) {
  ...
  i = b->size[0];
  b->size[0] = y->size[1];
  emxEnsureCapacity_real_T(&b_st, b, i, &vh_emlrtRTEI);
  b_data = b->data;
  bcoef = y->size[1];
  for (i = 0; i < bcoef; i++) {
  ...
  emxInit_real_T(&b_st, &x, 1, &ai_emlrtRTEI);
  i = x->size[0];
  x->size[0] = csz_idx_0;
  emxEnsureCapacity_real_T(&b_st, x, i, &wh_emlrtRTEI);
  x_data = x->data;
  if (csz_idx_0 != 0) {
    bcoef = (b->size[0] != 1);
    i = csz_idx_0 - 1;
  ...
  emxInit_real_T(&st, &b_y, 2, &yh_emlrtRTEI);
  ...
  if (X->size[0] != R_inv->size[1]) {
    if (((R_inv->size[0] == 1) && (R_inv->size[1] == 1)) ||
      ...
      emlrtErrorWithMessageIdR2018a(
          &b_st, &m_emlrtRTEI, "Coder:toolbox:mtimes_noDynamicScalarExpansion",
  ...
  e_mtimes(&b_st, R_inv, X, b_y);
  x_data = b_y->data;
  st.site = &ak_emlrtRSI;
  emxFree_real_T(&st, &R_inv);
  if (b_y->size[0] != b_y->size[1]) {
    emlrtErrorWithMessageIdR2018a(&st, &fb_emlrtRTEI, "Coder:MATLAB:square",
