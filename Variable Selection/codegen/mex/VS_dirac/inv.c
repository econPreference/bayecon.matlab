...
void inv(const emlrtStack *sp, const emxArray_real_T *x, emxArray_real_T *y)
{
  ...
  if ((x->size[0] == 0) || (x->size[1] == 0)) {
    ...
    emxInit_real_T(&st, &b_x, 2, &gc_emlrtRTEI);
    b_st.site = &ec_emlrtRSI;
    i = b_x->size[0] * b_x->size[1];
    b_x->size[0] = x->size[0];
    b_x->size[1] = x->size[1];
    emxEnsureCapacity_real_T(&b_st, b_x, i, &bc_emlrtRTEI);
    b_x_data = b_x->data;
    yk = x->size[0] * x->size[1];
    for (i = 0; i < yk; i++) {
      b_x_data[i] = x_data[i];
    }
    emxInit_int32_T(&b_st, &ipiv, 2, &gc_emlrtRTEI);
    emxInit_ptrdiff_t(&b_st, &ipiv_t, &hc_emlrtRTEI);
    emxInit_ptrdiff_t(&b_st, &r, &cc_emlrtRTEI);
    c_st.site = &kc_emlrtRSI;
    d_st.site = &mc_emlrtRSI;
    repmat(&d_st, (ptrdiff_t)0.0, muIntScalarMin_sint32(n, n), r);
    i = ipiv_t->size[0];
    ipiv_t->size[0] = r->size[0];
    emxEnsureCapacity_ptrdiff_t(&c_st, ipiv_t, i, &cc_emlrtRTEI);
    ...
    emxEnsureCapacity_int32_T(&c_st, ipiv, i, &dc_emlrtRTEI);
    ipiv_data = ipiv->data;
    d_st.site = &lc_emlrtRSI;
    emxFree_ptrdiff_t(&d_st, &r);
    if (yk < 0) {
      if (yk == -1010) {
    ...
    emxInit_int32_T(&c_st, &p, 2, &fc_emlrtRTEI);
    ...
    p->size[0] = 1;
    p->size[1] = x->size[0];
    emxEnsureCapacity_int32_T(&f_st, p, i, &ec_emlrtRTEI);
    p_data = p->data;
    p_data[0] = 1;
    yk = 1;
    ...
    if ((n1x == 0.0) || (n1xinv == 0.0) || (rc == 0.0)) {
      ...
      c_emlrt_marshallIn(&c_st, b_sprintf(&c_st, b_y, c_y, &m_emlrtMCI),
