...
void b_randn(const emlrtStack *sp, real_T varargin_1, emxArray_real_T *r)
{
  real_T *r_data;
  int32_T i;
  if (!(varargin_1 >= 0.0)) {
    emlrtNonNegativeCheckR2012b(varargin_1, &k_emlrtDCI, (emlrtCTX)sp);
  }
  i = r->size[0];
  r->size[0] = (int32_T)varargin_1;
  emxEnsureCapacity_real_T(sp, r, i, &jc_emlrtRTEI);
  r_data = r->data;
