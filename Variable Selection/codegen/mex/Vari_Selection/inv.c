...
void inv(const emlrtStack *sp, const emxArray_real_T *x, emxArray_real_T *y)
{
  ...
  if ((x->size[0] == 0) || (x->size[1] == 0)) {
    ...
    y->size[0] = x->size[0];
    y->size[1] = x->size[1];
    emxEnsureCapacity_real_T(sp, y, i, &cc_emlrtRTEI);
    y_data = y->data;
    ...
    y->size[0] = x->size[0];
    y->size[1] = x->size[1];
    emxEnsureCapacity_real_T(&st, y, i, &dc_emlrtRTEI);
    y_data = y->data;
    yk = x->size[0] * x->size[1];
    for (i = 0; i < yk; i++) {
      y_data[i] = 0.0;
    }
    emxInit_real_T(&st, &b_x, 2, &jc_emlrtRTEI);
    b_st.site = &jc_emlrtRSI;
    i = b_x->size[0] * b_x->size[1];
    b_x->size[0] = x->size[0];
    b_x->size[1] = x->size[1];
    emxEnsureCapacity_real_T(&b_st, b_x, i, &ec_emlrtRTEI);
    b_x_data = b_x->data;
    yk = x->size[0] * x->size[1];
    for (i = 0; i < yk; i++) {
      b_x_data[i] = x_data[i];
    }
    emxInit_int32_T(&b_st, &ipiv, 2, &jc_emlrtRTEI);
    emxInit_ptrdiff_t(&b_st, &ipiv_t, &kc_emlrtRTEI);
    c_st.site = &pc_emlrtRSI;
    d_st.site = &rc_emlrtRSI;
    e_st.site = &sc_emlrtRSI;
    i = ipiv_t->size[0];
    ipiv_t->size[0] = muIntScalarMin_sint32(n, n);
    emxEnsureCapacity_ptrdiff_t(&d_st, ipiv_t, i, &fc_emlrtRTEI);
    ...
    emxEnsureCapacity_int32_T(&c_st, ipiv, i, &gc_emlrtRTEI);
    ipiv_data = ipiv->data;
    ...
    if (yk < 0) {
      if (yk == -1010) {
        ...
        emlrtErrorWithMessageIdR2018a(
            ...
            "Coder:toolbox:LAPACKCallErrorInfo", 5, 4, 19, &fname[0], 12, yk);
      }
    }
    i = ipiv_t->size[0] - 1;
    for (k = 0; k <= i; k++) {
    ...
    emxInit_int32_T(&c_st, &p, 2, &ic_emlrtRTEI);
    ...
    p->size[0] = 1;
    p->size[1] = x->size[0];
    emxEnsureCapacity_int32_T(&f_st, p, i, &hc_emlrtRTEI);
    p_data = p->data;
    p_data[0] = 1;
    yk = 1;
    ...
    if ((n1x == 0.0) || (n1xinv == 0.0) || (rc == 0.0)) {
      ...
      c_emlrt_marshallIn(&c_st, b_sprintf(&c_st, b_y, c_y, &n_emlrtMCI),
